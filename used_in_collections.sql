...
WHERE (fullTitleEnglish LIKE '%Takami%' OR fullTitleJapanese LIKE '%Takami%') AND
 id IN (
	SELECT doujinId FROM doujin_tags
		EXCEPT
	SELECT doujinId FROM doujin_tags
...
SELECT * FROM doujin_details 
WHERE (fullTitleEnglish LIKE '%Cake%' OR fullTitleJapanese LIKE '%Cake%') AND
id IN (
	SELECT doujinId FROM doujin_tags
	WHERE doujin_tags.tagId IN (3) 
	GROUP BY doujinId
...
SELECT * FROM doujin_details 
WHERE (fullTitleEnglish LIKE '%(僕らのラブライブ%' OR fullTitleJapanese LIKE '%僕らのラブライブ%') AND
id IN (
	SELECT doujinId FROM doujin_tags
	WHERE doujin_tags.tagId IN (15) 
	GROUP BY doujinId
...
SELECT * FROM doujin_details 
WHERE (fullTitleEnglish LIKE '%LoveHala%' OR fullTitleJapanese LIKE '%LoveHala%') AND
id IN (
	SELECT doujinId FROM doujin_tags
	WHERE doujin_tags.tagId IN (50,2,3)  ----> Replace with list args
	GROUP BY doujinId
